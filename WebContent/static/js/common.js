...
//切换菜单选中样式
function menuChose(index){
	 for(var i=1;i<=6;i++){ 
        if(i==index){
           document.getElementById("menu"+i).className="menuCssChose";  
        }else{
           document.getElementById("menu"+i).className="menuCss"; 
        } 
      }
}
//返回顶部
var scrolltotop={ //页面出现滚动条后才出现“返回顶部”按钮功能代码
	//startline: Integer. Number of pixels from top of doc scrollbar is scrolled before showing control
	//scrollto: Keyword (Integer, or "Scroll_to_Element_ID"). How far to scroll document up when control is clicked on (0=top).
	setting: {startline:1, scrollto: 0, scrollduration:80, fadeduration:[500, 100]},
	controlHTML: '<div class="topcontrolCss"></div>', //HTML for control, which is auto wrapped in DIV w/ ID="topcontrol"
